...
class Model extends \Kotchasan\Model
{
  ...
  public function save(Request $request)
  {
    ...
    if ($request->initSession() && $request->isReferer()) {
      ...
      // ตรวจสอบค่าที่ส่งมา
      $ret = array();
      $antispam = new \Kotchasan\Antispam($request->post('write_antispamid')->toString());
      if (!$antispam->valid($request->post('write_antispam')->toString())) {
        // Antispam ไม่ถูกต้อง
        $ret['alert'] = 'กรุณากรอกตัวอักษรให้ถูกต้อง';
        $ret['input'] = 'write_antispam';
      } elseif ($save['detail'] == '') {
